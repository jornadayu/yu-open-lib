...
If a release is not being made in the same Pull Request as the story being added, then use the `"v1.4.4"` badge. Example:
...
export default {
  ...
  argTypes: { onClose: { action: 'onClose' } },
  parameters: {
    badges: ["v1.4.4"] // Component was still not released
  }
} as ComponentMeta<typeof DialogTitle>
```
